...
AI 에이전트 및 개발자가 새 코드를 작성할 때 따라야 할 프론트엔드 규칙입니다.
...
<Link href="/open-challenge">목록으로</Link>

// Bad
<a href="/open-challenge">목록으로</a>
```

## 4. 색상은 3.0 디자인 시스템 우선

3.0 토큰(`orange-1`~`orange-12`, `gray-1`~`gray-12`)을 우선 사용하고, 없을 경우에만 2.0 시맨틱 토큰(`text-text-main`, `border-line-line1` 등)을 사용합니다.
...
버튼, 인풋, 모달, 텍스트에어리어 등은 직접 구현하지 않고 `@/shared/components/ui`에서 import합니다.
...
## 10. 버튼에 `cursor-pointer` 적용

공통 `Button` 컴포넌트는 자동 적용되지만, 직접 구현한 `<button>`에는 반드시 `cursor-pointer`를 추가합니다.
...
`@/shared/components/ui`에서 import. 직접 구현 전 이 목록을 먼저 확인합니다.
...
| `Select`                                   | 드롭다운 선택 (폼용 h-56px / 필터용 `h-[36px]` override) |
| `SearchInput`                              | 검색 입력                                                |
| `Checkbox`                                 | 체크박스                                                 |
...
**빈 상태**: 데이터가 없을 때 간결한 텍스트 또는 아이콘 + 텍스트로 표시합니다.
